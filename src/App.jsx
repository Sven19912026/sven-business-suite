...
const navigation = [
  ...
  },
  {
    ...
    description: 'Laufzeiten und Fristen',
  },
  {
    title: 'Verhandlungen',
...
const moduleCards = [
  ...
  },
  {
    title: 'Verträge',
    icon: DescriptionRoundedIcon,
    eyebrow: 'Vertragsmanagement',
    description: 'Laufzeiten, Kündigungsfristen und Ansprechpartner zuverlässig im Blick behalten.',
    sourceKey: 'contracts',
    countLabel: 'aktiv',
    accent: '#7c3aed',
    soft: '#f2ecff',
...
const dashboardSources = [
  ...
  { key: 'contracts', collectionName: 'suiteVertraege', scoped: true },
      ...
      >
        ...
        <Box sx={{ position: 'relative', maxWidth: 690, py: 8 }}>
          ...
          <Typography sx={{ mt: 3, maxWidth: 610, opacity: 0.78, fontSize: '1.12rem', lineHeight: 1.75 }}>
            Aufgaben, Verträge, Verhandlungen, Geschäftskontakte und Mitarbeiter in einer modernen, synchronisierten Arbeitsoberfläche.
          </Typography>
...
function Dashboard({ user, openPage }) {
  ...
  const activeContracts = useMemo(
    () => contracts.filter((item) => item.status === 'Aktiv'),
    [contracts],
  )
...
function MobileNavigation({ page, setPage, openMore }) {
  const quickPages = ['Dashboard', 'Aufgaben', 'Verträge']
  ...
  return (
    ...
    >
      ...
      >
        ...
        <BottomNavigationAction label="Verträge" value="Verträge" icon={<DescriptionRoundedIcon />} />
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      >
        ...
        >
          <PageContent page={page} user={user} setPage={setPage} />
        </Box>
      </Box>
